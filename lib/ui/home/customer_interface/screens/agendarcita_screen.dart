...
class _AgendarCitaScreenState extends State<AgendarCitaScreen> {
  ...
  Future<void> _confirmarCita() async {
    ...
    if (modalidadSeleccionada == "Domicilio" &&
    ...
    final pets =
        await petController.getPetsStream(FirebaseAuth.instance.currentUser!.uid).first;
    ...
    final double latitudCita =
        modalidadSeleccionada == "Presencial" ? clinicaLat! : ubicacionLat!;
    ...
    final cita = CitaModel(
      ...
      latitud: latitudCita,
      longitud: longitudCita,
      estado: "pendiente",
      pagado: false,
      observaciones: observacionesCtrl.text.trim(),
      modalidad: modalidadSeleccionada ?? "Presencial",
    );

    try {
      final idCita = await appointmentController.crearCita(cita);
      await FirebaseFirestore.instance.collection('appointments').doc(idCita).update({
            ...
            StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapClinicas) {
                ...
                final clinicNames = docs
                    .map<String>(
                        (d) => (d.data() as Map<String, dynamic>)['nombre']?.toString() ??
                            'Veterinaria')
                    .toList();
                  ...
                  children: [
                    CitaDropdownField(
                      ...
                      onChanged: (v) {
                        ...
                        for (var d in docs) {
                          final name = (d.data() as Map<String, dynamic>)['nombre']
                                  ?.toString() ??
                              '';
                          if (name == v) {
                            match = d;
                        ...
                        if (match != null) {
                          ...
                          setState(() {
                            ...
                            clinicaLat = (data['latitud'] ?? 0).toDouble();
                            clinicaLng = (data['longitud'] ?? 0).toDouble();

                            direccionVeterinaria = data['direccion']?.toString();
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          selectedClinicId == null
                              ? _campoDeshabilitado(
                                  'Tipo de Servicio', 'Selecciona primero una clínica')
                              : _dropServicios(),
                          ...
                          selectedClinicId == null
                              ? _campoDeshabilitado('Veterinario (Opcional)',
                          ...
                          CitaDropdownField(
                            ...
                            onChanged: (v) async {
                              ...
                              if (v == "Presencial") {
                                setState(() {
                                  ubicacionLat = clinicaLat;
                                  ubicacionLng = clinicaLng;
                                  direccionUsuario = null;
                                });
                              } else if (v == "Domicilio") {
                                try {
                                  final pos = await _obtenerUbicacionCliente();
                                  setState(() {
                                    ubicacionLat = pos.latitude;
                                    ubicacionLng = pos.longitude;
                                    direccionUsuario = "Ubicación del cliente";
                                  });
                                } catch (e) {
                                  Get.snackbar("Error ubicación", e.toString());
                                }
                              }
                            },
                          ),
                            ...
                            Column(
                              ...
                              children: [
                                ElevatedButton.icon(
                                  ...
                                  label:
                                      const Text("Ver ubicación en Google Maps"),
  ...
  Widget _dropServicios() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapServices) {
        ...
        final serviceItems = docsS.map<DropdownMenuItem<String>>((d) {
        ...
        if (serviceItems.isEmpty) {
          return _campoDeshabilitado('Tipo de Servicio', 'No hay servicios disponibles');
        }
        ...
        return Padding(
          ...
          child: DropdownButtonFormField<String>(
            ...
            onChanged: (val) {
              if (val == null) return;
              final matched = docsS.firstWhere((d) => d.id == val);
              ...
              setState(() {
                ...
                servicioSeleccionadoNombre = "$name — \$$price";
                precioServicioSeleccionado = double.tryParse(price) ?? 0.0;
              });
            },
          ),
        );
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapVets) {
        ...
        final vetItems = docsV.map<DropdownMenuItem<String>>((d) {
          ...
          return DropdownMenuItem(
            value: d.id,
            child: Text(fullName.isNotEmpty ? fullName : 'Veterinario'),
          );
        }).toList();

        if (vetItems.isEmpty) {
          return _campoDeshabilitado(
              'Veterinario (Opcional)', 'No hay veterinarios disponibles');
        }
        ...
        return Padding(
          ...
          child: DropdownButtonFormField<String>(
            ...
            onChanged: (val) {
              if (val == null) return;
              final matched = docsV.firstWhere((d) => d.id == val);
              final data = matched.data() as Map<String, dynamic>;
              final fullName =
                  '${(data['nombre'] ?? '').toString()} ${(data['apellido'] ?? '').toString()}'
                      .trim();
